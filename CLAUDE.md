...
- **Pay advances** (earned wage access, `AppState.advance`). The user's confirmed PayActiv setup, which is the default: **$3 flat fee, money arrives instantly, at most half a paycheck (`maxPercent` = 50% of `income.payAmount`) per pay period**, repaid + fee from the **next** paycheck; the limit resets when a paycheck repays it (`advanceAvailable()` = limit − advances still outstanding). Public PayActiv info differs by employer (fees $0–$4.99, free 1–3 day bank transfer, availability accruing daily as you earn) so everything is configurable; `limitToEarned` (off by default) additionally caps availability at that % of wages earned so far. Don't reintroduce a $ cap or assume the general web numbers over the user's. `buildPlan()` first simulates without advances, then loops `improveWithAdvance()`: for the most urgent payment that's still late (or paid via a fee-charging overdraft) it tries a new advance requested `leadDays` before the deadline — or a top-up of an existing one (one fee, not two) — **re-simulates**, and keeps it only if `score()` improves (fees + late fee/`LATE_PENALTY` per miss + huge penalty for missing a bureau deadline). That guard matters: an advance shrinks the next paycheck and can push a later payment late. `advancesValid()` re-checks employer limits for every advance whenever one changes. Free overnight overdraft beats an advance; a fee-charging overdraft loses to it. Paychecks repay owed advances (recorded `income.advances` + planned, fee included), carrying any remainder to the next check.
