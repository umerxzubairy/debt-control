...
- **Pay advances (PayActiv / earned wage access)** — configure your flat fee, how fast the money arrives, and your per-pay-period limit (default: $3, instant, up to half a paycheck), and the plan takes an advance on the last safe day whenever that lets a payment land on time. It sizes the advance to what's missing, tops up an existing one instead of paying a second fee, accounts for the repayment shrinking your next paycheck, and skips advances that would only shift the problem. Fees and limits vary by employer (see [PayActiv pricing](https://payactiv.com/program-pricing) and [Paychex Pay On Demand](https://payactiv.com/paychex/)), so every value is editable.
- **Your real pay schedule** — every N days, or fixed days each month with a work period per paycheck (e.g. paid the 1st for the 8th–23rd, the 15th for the 23rd–8th).
